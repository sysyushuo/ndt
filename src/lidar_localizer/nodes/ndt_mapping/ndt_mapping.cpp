...
static std::string  _lidar_topic_,_pcd_path_;
...
void save_nd_map(std::string pcd_name);
...
static ros::Publisher localizer_pose_pub, ndt_pose_pub, odom_pub,odom_path_pub,ndt_point_pub;
static nav_msgs::Path globalPath;
static geometry_msgs::PoseStamped localizer_pose_msg, ndt_pose_msg;
static geometry_msgs::PointStamped ndt_point_msg;
...
ndt_point_msg.header.frame_id = "map";
ndt_point_msg.header.stamp = current_scan_time;
ndt_point_msg.point.x=global_t2(0, 3);
ndt_point_msg.point.y=global_t2(1, 3);
ndt_point_msg.point.z=global_t2(2, 3);


  nav_msgs::Odometry laserOdometry;
  laserOdometry.header.frame_id = "map";
  laserOdometry.header.stamp = current_scan_time;
  laserOdometry.header.stamp = current_scan_time;
  laserOdometry.pose.pose.position.x = global_t2(0, 3);
  laserOdometry.pose.pose.position.y = global_t2(1, 3);
  laserOdometry.pose.pose.position.z = global_t2(2, 3);
  laserOdometry.pose.pose.orientation.x = ndt_q.x();
  laserOdometry.pose.pose.orientation.y = ndt_q.y();
  laserOdometry.pose.pose.orientation.z = ndt_q.z();
  laserOdometry.pose.pose.orientation.w = ndt_q.w();
  odom_pub.publish(laserOdometry);

  geometry_msgs::PoseStamped pose_stamped;
  pose_stamped.header.stamp = current_scan_time;
  pose_stamped.header.frame_id = "map";
  pose_stamped.pose.position.x = global_t2(0, 3);
  pose_stamped.pose.position.y = global_t2(1, 3);
  pose_stamped.pose.position.z = global_t2(2, 3);
  pose_stamped.pose.orientation.x = ndt_q.x();
  pose_stamped.pose.orientation.y = ndt_q.y();
  pose_stamped.pose.orientation.z = ndt_q.z();
  pose_stamped.pose.orientation.w = ndt_q.w();
  globalPath.poses.push_back(pose_stamped);
  globalPath.header.stamp = current_scan_time;
  globalPath.header.frame_id = "map";
  odom_path_pub.publish(globalPath);


  localizer_pose_pub.publish(localizer_pose_msg);
  ndt_pose_pub.publish(ndt_pose_msg);
  ndt_point_pub.publish(ndt_point_msg);
...
void save_path_file(const std::string &filename)
{
    std::cout << "write history file, size: " << globalPath.poses.size() << " ..." << std::endl;
   ...
   std::ofstream f1(filename.c_str());          
    ...
    {   
        auto path_point = globalPath.poses.at(i);
          f1<<std::fixed<<std::setprecision(3)<<path_point.header.stamp.sec<<"  "<<0<<" "
          <<path_point.pose.position.x<<"  "<<path_point.pose.position.y<<" "
          <<path_point.pose.position.z<<" "<<path_point.pose.orientation.w<<" "
          <<path_point.pose.orientation.x<<" "<<path_point.pose.orientation.y<<" "
          <<path_point.pose.orientation.z<<std::endl; 
    }
    f1.close(); 
...
nh.getParam("lidar_topic", _lidar_topic_);
// std::cout<<_lidar_topic_<<std::endl;
nh.getParam("pcd_path", _pcd_path_);
  ...
  ndt_pose_pub = nh.advertise<geometry_msgs::PoseStamped>("/current_pose", 1000);
  ndt_point_pub = nh.advertise<geometry_msgs::PointStamped>("/ndt_point", 1000);
  localizer_pose_pub = nh.advertise<geometry_msgs::PoseStamped>("/localizer_pose", 1000);
  odom_pub = nh.advertise<nav_msgs::Odometry>("lidar_odom", 10);
  odom_path_pub = nh.advertise<nav_msgs::Path>("lidar_odom_path", 1);
  // ros::Subscriber points_sub = nh.subscribe("points_raw", 1000, points_callback);
  ros::Subscriber points_sub = nh.subscribe(_lidar_topic_.data(), 1000, points_callback);
  ros::spin();
  ...
  bool path_store_flag;
  ros::param::get("path_store_flag", path_store_flag);
  std::cout<<"path_store_flag is:"<<path_store_flag<<std::endl;
  if(path_store_flag) {
    save_path_file(file_name);
    save_nd_map(_pcd_path_);
  }
  return 1;
}